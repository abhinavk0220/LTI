...
Sequential console app implementing:
 - Load security_incidents.txt (one incident per line)
 - Chunk using RecursiveCharacterTextSplitter
 - Create embeddings (sentence-transformers) and FAISS index (if available) or numpy fallback
 - Build hybrid retriever (vector similarity + BM25/TF-IDF)
 - LCEL chain: retrieve -> prompt -> Ollama (mistral) -> parser
 - RunnableWithMessageHistory for per-analyst memory
 - Entity extraction (IP, OS, Hostname, MITRE Txxxx, Severity)
 - Threat scoring, simple threat enrichment tool
