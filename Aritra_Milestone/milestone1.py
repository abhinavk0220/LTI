...
prompt = ChatPromptTemplate.from_messages([ ("system", """You are a Security Operations Center (SOC) analyst assistant. Your role is to help analysts investigate security incidents using the provided context.

Context from Security Incidents Database: {context}
...
Maintain professional security analyst tone"""), ("placeholder", "{history}"), ("human", "{question}") ])
...
def format_entities(entities_dict): 
    if isinstance(entities_dict, dict) and entities_dict: return "\n".join([f"- {k}: {v}" for k, v in entities_dict.items()]) 
    return "No entities extracted"

def get_question_string(input_dict): 
    if isinstance(input_dict, dict): return input_dict.get("question", "") 
    return str(input_dict)

chain = { "context": lambda x: format_docs(hybrid.invoke(get_question_string(x))), "question": get_question_string, "entities": lambda x: format_entities(x.get("entities", {}) if isinstance(x, dict) else {}), } | prompt | llm | parser
...
def threat_enrich(ip: str):
    ...
    mock_threat_db = { "10.1.1.9": "Known SSH brute-force source | Reputation: Malicious | First seen: 2024-01", "10.2.4.9": "SSH attack source | ASN: AS12345 | Country: Unknown", "172.22.9.54": "Port scanning activity detected | Threat Score: 8/10", "10.22.3.9": "C2 communication IP | Malware family: Cobalt Strike", "192.168.55.20": "Blacklisted country source | VPN exit node suspected" } 
...
def get_session_history(session_id: str): # TODO: Create InMemoryChatMessageHistory if missing # TODO: Return the history object 
    if session_id not in store: 
        store[session_id] = InMemoryChatMessageHistory() 
    return store[session_id]

# TODO: Wrap chain with RunnableWithMessageHistory
memory_chain = RunnableWithMessageHistory( chain, get_session_history, input_messages_key="question", history_messages_key="history" ) 
...
def extract_entities(query: str, context: str): 
    """Students: Extract IPs, hostname, OS, MITRE tags, severity""" # TODO: Your custom extraction logic 
    entities = {}
...
def calculate_threat_score(entities: dict, context: str) -> dict: 
    """ Calculate threat score based on: - MITRE tag severity - Severity level - Presence of malicious IP - Suspicious PowerShell activity - Brute-force indicators """
    score = 0 
    factors = []
...
def parse_to_json(response: str, entities: dict, threat_score: dict) -> dict: 
    ...
    structured_output = { "response": response, "entities": entities, "threat_analysis": { "score": threat_score["score"], "level": threat_score["threat_level"], "risk_factors": threat_score["factors"] }, "metadata": { "incident_count": response.count("Incident #"), "has_resolution": "resolution" in response.lower() or "resolved" in response.lower(), "mitre_techniques": entities.get("MITRE", "None").split(", ") if "MITRE" in entities else [] } }
